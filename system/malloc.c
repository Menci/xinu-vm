 ...
 * Realloc is not implemented indirectly using malloc and free.
 *
...
// Disable debugging
#define assert(...) ((void)NULL)
#define fprintf(...) ((void)NULL)

// Ports for Xinu heap
#define HEAP_SIZE       (processTable[currentProcessID].heapSize)
#define HEAP_LOW        VM_HEAP_START_PAGE_ADDRESS
#define HEAP_HIGH       (HEAP_LOW + HEAP_SIZE)
...
inline static size_t get_block_length(block_t block) {
    return read_24_transformed(&block[0]);
}

inline static bool get_block_is_free(block_t block) {
    return read_bit(&block[0]);
}

inline static void set_block_length_footer(block_t block, size_t value) {
    uint32_t transformed_value = value >> 2;
    write_24_preserve_low_bit(&block[value - 3], transformed_value);
}

inline static void set_block_length(block_t block, size_t value) {
    assert(value != 0);
...
inline static void set_block_is_free_flag_only(block_t block, bool value) {
    write_bit(&block[0], value);
}

inline static void init_block_header_type_bit(block_t block) {
...
inline static void *get_block_payload(block_t block) {
    if (get_block_type(block) == BLOCK_8_ALIGNED) {
...
inline static block_t get_payload_block(void *payload) {
...
inline static uint8_t *get_block_header_second_part_offseted(block_t block) {
...
// index: 1 ~ 3
inline static uint32_t read_block_header_integer(block_t block, size_t index) {
    if (get_block_type(block) == BLOCK_8_ALIGNED) {
...
// index: 1 ~ 3
inline static void write_block_header_integer(block_t block, size_t index, uint32_t value) {
    uint32_t transformed_value = value >> 2;
    if (get_block_type(block) == BLOCK_8_ALIGNED) {
        uint8_t *second_part = get_block_header_second_part_offseted(block);
...
// index: 1 ~ 2
inline static uint32_t read_block_header_bit(block_t block, size_t index) {
    switch (index) {
    case 1: // 3
...
// index: 1 ~ 2
inline static void write_block_header_bit(block_t block, size_t index, bool value) {
    // 8-aligned
    switch (index) {
    case 1: // 3
...
#define rbroot (*(rb_node_t *)HEAP_LOW)
// Store the null node in the following 12 bytes
#define rbnull ((rb_node_t)(sizeof(rb_node_t) + (uint8_t *)HEAP_LOW))

inline static void *rb_init() {
...
inline static void rb_init_node(rb_node_t node) {
    // A node not in the tree is indicated by having itself as its parent.
    rb_set_parent(node, node);
}

inline static void rb_rotate(rb_node_t node, rb_which_t which, rb_node_t old_parent, rb_which_t old_parent_which, rb_node_t old_grandparent) {
...
inline static void rb_insert_fixup(rb_node_t node) {
    while (true) {
        if (node == rbroot) {
...
inline static void rb_insert(rb_node_t node) {
#ifdef DEBUG
...
inline static void rb_delete_fixup(rb_node_t node, rb_node_t parent, rb_which_t which) {
    while (parent != rbnull) {
...
inline static void rb_delete_with_at_most_one_child(rb_node_t child, rb_node_t parent, rb_which_t which, rb_color_t color) {
...
inline static void rb_delete(rb_node_t node) {
#ifdef DEBUG
...
inline static rb_node_t rb_lower_bound(size_t value) {
...
#ifdef DEBUG
inline static void rb_traversal_indent(size_t depth) {
    for (size_t i = 0; i < depth * 2; i++) putchar(' ');
}

inline static size_t rb_traversal(rb_node_t node, size_t depth, bool print, bool validate, size_t **values) {
...
#define available_heap_size \
    (HEAP_SIZE - HEAP_HEADER_SIZE)

#define is_last_block(block, length) \
    ((void *)((block) + (length) - 1) == HEAP_HIGH)

#define virtual_block_to_indicate_if_the_last_block_is_free \
    ((block_t)rbnull)

inline static block_t get_previous_block_if_free(block_t block) {
...
inline static block_t get_next_block_if_free(block_t block) {
    size_t length = get_block_length(block);
...
inline static block_t get_logical_next_block(block_t block) {
    size_t length = get_block_length(block);
...
inline static void set_block_is_free(block_t block, bool is_free) {
...
inline static void init_block(
    ...
    assert(length >= MINIMAL_BLOCK_SIZE && length < HEAP_SIZE);
...
inline static block_t extend(size_t wanted_block_size) {
    block_t new_space = 1 + (block_t)HEAP_HIGH;
    ...
    if (old_last_block_is_free) {
        ...
        changeHeapSize(wanted_block_size - last_block_length);
        init_block(last_block, wanted_block_size, true, PRESERVE, true);

        return last_block;
    } else {
        changeHeapSize(wanted_block_size);
...
inline static int malloc_init(void) {
    changeHeapSize(HEAP_HEADER_SIZE + INIT_HEAP_AVAILABLE_SIZE);

    rb_init();

    uint8_t *heap = (uint8_t *)HEAP_LOW;
...
void *malloc(size_t request_size) {
    ...
    assert(payload > HEAP_LOW && payload < HEAP_HIGH);
...
void free(void *ptr) {
    ...
    assert(ptr > HEAP_LOW && ptr < HEAP_HIGH);
...
inline static void *mm_realloc_by_malloc_free(void *ptr, size_t size) {
    ...
    if (size == old_payload_size) {
        ...
        if (is_last_block(old_block, get_block_length(old_block))) {
            ...
            if (next_block && is_last_block(next_block, next_block_length) && next_block_length <= min_grow_size) {
                ...
                rb_delete(next_block);
                changeHeapSize(min_grow_size - next_block_length);
                new_block = old_block;
                init_block(new_block, old_block_length + min_grow_size, false, PRESERVE, true);
                new_payload = old_payload;
            } else {
                ...
                if (merge_previous_block || merge_next_block) {
                    ...
                    if (new_block != old_block) {
                        moveMemory(new_payload, old_payload, old_payload_size);
...
#ifdef DEBUG
inline static void mm_validate_blocks() {
    block_t block = (block_t)HEAP_LOW + HEAP_HEADER_SIZE;
    while (block != (block_t)((uint8_t *)HEAP_HIGH + 1)) {
        assert(get_block_is_free(block) == get_previous_is_free(get_logical_next_block(block)));
        block += get_block_length(block);
    }
}

inline static void mm_print_blocks() {
    block_t block = (block_t)HEAP_LOW + HEAP_HEADER_SIZE;
    size_t i = 0;
    while (block != (block_t)((uint8_t *)HEAP_HIGH + 1)) {
