	...
	)				/*   to accept commands		*/
{
	...
	while (TRUE) {
		...
		allocateVirtualMemoryPages(processTable[child].pageDirectoryPhysicalAddress, VM_SHELL_ARGUMENT_PAGE_ID, 1);
		writeToAnotherVirtualMemorySpacePage(processTable[child].pageDirectoryPhysicalAddress, VM_SHELL_ARGUMENT_PAGE_ID, argumentsPageContent);
		...
		processTable[child].descriptors[0] = stdinput;
		processTable[child].descriptors[1] = stdoutput;
